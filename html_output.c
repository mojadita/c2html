...
/* functions */
FILE *html_create(const node *file)
{
	node *hf = file->html_file;
	FILE *res;
	int i, j;
	char *typ;

	res = hf->index_f = fopen (file->html_file->full_name, "w");
	if (!hf->index_f) {
		fprintf(stderr, PROGNAME": "__FILE__"(%d):fopen:%s:%s(errno=%d)\n",
			__LINE__, hf->full_name, strerror(errno), errno);
		exit(EXIT_FAILURE);
	} /* if */
...
"    <div class=\"title\">%s %s</div>\n",
	typ, file->full_name, typ, file->name);
	fprintf(res, "    <ul>\n");
	return res;
} /* create_html */
